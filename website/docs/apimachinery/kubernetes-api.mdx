...
不管是客户端工具（`kubectl`），UI(`dashboard`)，还是Kubernetes系统内部组件（`kubelet`，内置的*控制器（Controller）*[^4]等）在背后，它们最终仍然指向Kubernetes API。
:::
...
在文档[Kubernetes API Conventions <GithubSVG />](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md)中，我们可以找到官方社区对*Kubernetes资源*的一种描述：
...
当然，文档[Kubernetes API Conventions <GithubSVG />](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md)对*Kubernetes资源*的描述还是稍显模糊。
...
至于为什么用一个版本字段同时记录分组以及版本信息最早可以追溯到引入API分组的提案：[<GithubSVG /> Pull Request #12951](https://github.com/kubernetes/kubernetes/pull/12951)。
...
* *kind*是*Kubernetes资源*JSON格式在序列化前对应的Go类型。当然，如果不局限于具体语言，我们也可以把*kind*认为是JSON的Schema[^14]。
:::
...
关于声明式系统和命令式系统完整的介绍以及与Kubernetes的联系可以参考Kubernetes文档特别兴趣小组成员[Andrew Chen](https://github.com/kubernetes/community/blob/master/sig-docs/README.md#emeritus-leads)的博文: [Imperative vs Declarative](https://dominik-tornow.medium.com/imperative-vs-declarative-8abc7dcae82e)。

</details>


根据Kubernetes设计文档[The Kubernetes Resource Model (KRM) <GithubSVG />](https://github.com/kubernetes/design-proposals-archive/blob/main/architecture/resource-management.md)，Kubernetes被设计成一个声明式资源管理系统。
...
#### spec和status
几乎所有的Kubernetes对象都包含两个字段：`spec`和`status`。用户/客户端在创建对象时，`spec`字段用以描述资源的期望状态；
...
按照社区的说法[^17]，这样做的原因是为了在风格上保持一致；
:::


:::tip 小节
至此，我们知道Kubernetes资源可以分为两类：*Kubernetes对象*以及*Kubernetes虚拟资源*；

尽管*资源*包括*Kubernetes对象*，*资源*的概念更倾向于在Kubernetes API上下文中使用[^18]，
而*Kubernetes对象*更被侧重用于对集群期望状态的一种描述。
在不严格区分定义的情况下，*资源*和*Kubernetes对象*两个概念往往可以被互换使用。
:::
下图总结了Kubernetes资源和Kubernetes对象的区别和联系：
...
在这个讨论里，官方开发者们提出了很多想法，他们最终采纳了*子资源*的方式——即在普通资源对应的API URL后追加一个路径片段`/status`暴露一个资源的"子"API，同时这个"子"API仅能更新资源的`status`字段而忽略掉其他字段（`spec`）；
*子资源*的[<GithubSVG /> Pull Request #4779](https://github.com/kubernetes/kubernetes/pull/4779)被合并，并在[Kubernetes v0.13.0](https://github.com/kubernetes/kubernetes/tree/v0.13.0)中发布；
:::
...
[^17]: [API Conventions <GithubSVG />](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/api-conventions.md)
...
[^18]: [The API Object Lifecycle <GithubSVG />](https://github.com/kubernetes/community/blob/master/contributors/devel/sig-architecture/object-lifecycle.md)
       >
...
[^2]:
      你可以在Kubernetes API对应的[OpenAPI](https://www.openapis.org/) [specification文件 <GithubSVG />](https://github.com/kubernetes/kubernetes/blob/release-1.27/api/openapi-spec/swagger.json)中找到所有API的schema。
      另外，[Kubernetes API reference <KubernetesSVG />](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.27/)以文档的形式记录所有了所有API的详细说明。
...
[^2]: 我们以[Kubernetes v1.0.0](https://github.com/kubernetes/kubernetes/tree/v1.0.0)为例，在[Kubernetes v1.0.0](https://github.com/kubernetes/kubernetes/tree/v1.0.0)时仍有部分API未演化成稳定版本，以`api/v1beta3/`为根路径，参考[API swagger spec](https://github.com/kubernetes/kubernetes/blob/v1.0.0/api/swagger-spec/v1beta3.json)。
[^3]: 在提案[api-group](https://github.com/kubernetes/kubernetes/blob/v1.1.0-beta/docs/proposals/api-group.md#bookkeeping-for-groups)中，作者写到：
      > For a fast implementation targeting the v1.1 deadline, we will concatenate group with version, in the form of "group/version", and use it where a version string is expected, so that many code can be reused.
[^3]:
...
[^10]: Kubernetes中存在一小部分资源类型并不对应集群中具体的实体，这些资源被成为*虚拟资源（virtual resource）*，
       但是根据我们先前的定义，它们有对应的Kubernetes API Path，有对应的Go类型，可以被序列化为JSON格式的字节数据在HTTP中传输，所以它仍然符合*Kubernetes资源*的范畴。
